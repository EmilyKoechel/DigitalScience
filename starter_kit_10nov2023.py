# -*- coding: utf-8 -*-
"""Starter Kit 10NOV2023

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1eldhiymqkDpcJXfUQHuaKC7ubzArg590
...
##  DIMCLI Documentation (also some non-Colab access options here): https://digital-science.github.io/dimcli/getting-started.html
...
#Identify the number of publications per year with the keyword "Octopus"
# %dsldf search publications for "Octopus" return year

#Which early career researchers publish the most papers about Octopuses in recent years??
# %dsldf search publications for "Octopus" where year > 2021 return researchers[id+first_name+last_name+first_publication_year]
##Returns ALL researchers who write about octopus and the info needed to filter to ECR

#Which SDGs are associated with the most US funding on grants about Octopuses?
# %dsldf search grants for "Octopus" where funder_org_countries = "US" return category_sdg aggregate count, funding sort by funding

#for multi-level grouping, such as to identify time trends of the top SDGs by funding by year,
#see "Multi-Level Grouping" section below.
...
#SQL friends - this is a case statement!
...
select OA, CatFOR, Year, sum(distinct_pubid) from API
group by OA, CatFor, Year
...
"""##Dataset Citations (Workaround)"""

#Datasets datasource doesn't have a citations field, however, you can use the full-text search to identify
#publications that reference the DOI of the dataset.

DS = %dsldf search datasets where research_orgs = "grid.6572.6" and year = 2018 return datasets[id+doi+title+authors] limit 10
DSDOIList = list(dict.fromkeys(DS['doi'].dropna().tolist()))
print(DSDOIList)

q_template = """
search publications for "{}" return publications[id]
"""
# TODO escape ^ " : ~ \ [ ] { } ( ) ! | & +
#FOR COUNTS ONLY:
out = pd.DataFrame()
for x in DSDOIList:
    ...
    RecordCount = dsl.query(q).count_total
    data = {'DS_DOI': [x], 'Count': [RecordCount]}
...
#Join and merge (The next cell produces the same results using embedded SQL)
#Summarize by Cited ID and Citing Publication Year


JOINED = pd.merge(
    CRBPubs.rename(columns={"id": "citedid", "times_cited": "TotalCitations", "year": "PublicationYear"}),
    CitingPub.rename(columns={"id": "citingid", "year": "citation_year"}),
    left_on="citedid",
    right_on="reference_ids",
    how="left"
)
JOINED = JOINED.sort_values(by="citedid")

print(len(JOINED), "Author/Publication/Most Relevant Concept Combinations")
print(JOINED.head(100))

# Grouping and aggregating
JOINEDandGrouped = JOINED.groupby(["citedid", "PublicationYear", "citation_year"]).agg({"citingid": "nunique"})
JOINEDandGrouped = JOINEDandGrouped.rename(columns={"citingid": "CitationCount"}).reset_index()
JOINEDandGrouped = JOINEDandGrouped.sort_values(by="citedid")

print(len(JOINEDandGrouped), "Citations by Year")
print(JOINEDandGrouped.head(100))

#For those more comfortable with SQL, you can write SQL against Pandas Dataframes!
#This cell is in lieu of the one above.
